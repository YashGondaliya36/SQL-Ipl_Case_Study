...
-- 4) Top 5 team score
		...
		join matches m on t.id = m.id order by t.Total_runs desc;
...
-- 12) innings taken by given batter to complete given total run
 ...
 -- # 14) Most maiden over bowled in Indian Premier League
		
		select count(overs) from balls  group by overs ;
...
-- 32) On which day virat kohli scored century
	...
	select * from (select distinct m.id,dayname(date),sum(batsman_run) over( partition by m.id) AS "RUN" from matches m
